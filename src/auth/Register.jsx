    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
        ...
        <form onSubmit={onSubmit} className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-lg dark:border-zinc-800 dark:bg-zinc-900">
          <div className="space-y-4">
            <div>
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-xl border border-zinc-300 bg-transparent px-3 py-2 outline-none focus:border-indigo-400 dark:border-zinc-700" placeholder="Adınız" />
            </div>
            <div>
        ...
        <div className="mt-4 text-sm text-zinc-600 dark:text-zinc-300">
          Zaten hesabın var mı? <Link to="/login" className="text-indigo-600 hover:underline">Giriş Yap</Link>
        </div>
      </motion.div>
    </div>
  )
